...
for index in range(0,3):
    ...
    for k in range(5,6,1):
        vocab_size = 4 ** k
        print('Verificando eficácia do vocab_size = ' + str(vocab_size))
        data = np.loadtxt(pathDict[index] + 'word-' + str(vocab_size) + '-bpe-'+ str(WINDOW_SIZE) +'-win-' + str(VECTOR_SIZE) + '-size.csv', dtype=np.float64, delimiter=',')
    ...
    print('Imprimindo estatísticas para representação em Word2Vec de bpe.')
    # printar a lista em ordem
    resultFile = open(pathDict[index] + 'word-' + str(WINDOW_SIZE) + '-win-' + str(VECTOR_SIZE) + '-siz-results-bpe.txt','w')
