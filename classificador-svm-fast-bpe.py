...
for index in range(0,3):
    ...
    for k in range(5,6,1):
        ...
        data = np.loadtxt(pathDict[index] + 'fast-' + str(vocab_size) + '-bpe-'+ str(WINDOW_SIZE) +'-win-' + str(VECTOR_SIZE) + '-size.csv', dtype=np.float64, delimiter=',')
    ...
    print('Imprimindo estatísticas para representação em FastText de bpe.')
    # printar a lista em ordem
    resultFile = open(pathDict[index] + 'fast-' + str(WINDOW_SIZE) + '-win-' + str(VECTOR_SIZE) + '-siz-results-bpe.txt','w')
