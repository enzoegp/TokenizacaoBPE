...
for k in range(5,6,1):
    ...
    for index in range(0,3):
        ...
        model = FastText.load(pathDict[index]+"model-fast-"+str(vocab_size)+"-bpe-"+str(windowSize)+"-win-"+str(vectorSize)+"-size.model")

        # abre o arquivo no qual escreve as sequências de vetores
        vectorFile = open(pathDict[index]+"fast-"+str(vocab_size)+"-bpe-"+str(windowSize)+"-win-"+str(vectorSize)+"-size.csv",'w')
